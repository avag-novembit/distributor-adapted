...
function send_notifications( $post_id ) {
	// current_user_can is not working, when running cron in background.
	...
	foreach ( $subscriptions as $subscription_key => $subscription_id ) {
		...
		if ( ! is_wp_error( $request ) ) {
			...
			$return[ $subscription_id ] = json_decode( wp_remote_retrieve_body( $request ) );
		} else {
			error_log( sprintf( 'Something went wrong distributing post \'%s\' (%d) to %s. More details logged below.', $post->post_title, $post->ID, $target_url ) );
			error_log( print_r( $request, true ) );

			$return[ $subscription_id ] = $request;
		}
	}
